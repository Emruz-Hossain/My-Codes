...
ll rec(ll idx,ll prev)
{
    ...
    if(prev==0)
    {
        ret=(rec(idx,1)*(prev==0?color:color-1))%mod;
    }
    else
    {
        ret=(rec(idx+1,0)*(prev==0? color-1:color-2))%mod;
    }
    return ret;
}
int main()
{
   ...
   for(x=1;x<=t;x++)
   {
       ...
       {
          if(i%2)
            ar[i]=(ar[i-1]*(color-1))%mod;
           else
           {
              ar[i]=(ar[i-1]+((((ar[i-1]-ar[i-2])%mod +mod)%mod)*(color-2))%mod)%mod;
           }
       }
       pf("%lld\n",(color*ar[2*n])%mod);
   }
