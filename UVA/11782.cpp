...
int idx;
int tree[1048578],ar[1048577],dp[21][1048577];
void built(int rt,int level)
{
    if(level<0)
        return;
    tree[rt]=ar[idx++];
    built(2*rt,level-1);
    built(2*rt+1,level-1);
}
int rec(int rt,int rem,int level)
{
    int i,sm;
    if(rem<=1||level==0)
        return tree[rt];
    ...
    for(i=1;i<rem;i++)
    {
        sm=max(sm,rec(2*rt,i,level-1)+rec(2*rt+1,rem-i,level-1));
    }
    sm=max(sm,tree[rt]);
    return ret=sm;
}
