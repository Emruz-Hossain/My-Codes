...
unsigned long long ar[]={0,1,2,2,6,7};
unsigned long long cal(ll n)
{
    if(n==0LLU)
        return 0;
    if(n%8==6LLU||n%8==7LLU)
        return 0;
    else if(n%8==2LLU||n%8==3LLU)
        return 2;
    else
    {
        unsigned long long tmp=n%8;
        n=n-tmp;
        n=n/8;
        return n*8+ar[tmp];
    }
...
int main()
{
   ...
   unsigned long long a,b,c,d,h,m,n,p,x,y,i,j,k,l,q,r,t,cnt,sm,tmp;
   sf("%llu",&q);
  for(i=1;i<=q;i++)
   {
       sf("%llu %llu",&l,&r);
