...
int main()
{
   ...
   for(i=0;i<515;i++)
   {
       ...
       {
           dp[p].S[j].clear();
           for(itt=dp[!p].S[j].begin();itt!=dp[!p].S[j].end();++itt)
           {
               if(dp[!p].S[j+1].find(*itt)==dp[!p].S[j+1].end())
                dp[p].S[j].insert(*itt);
           }
           for(itt=dp[!p].S[j+1].begin();itt!=dp[!p].S[j+1].end();++itt)
           {
               if(dp[!p].S[j].find(*itt)==dp[!p].S[j].end())
                dp[p].S[j].insert(*itt);
           }
           pf("    ");
           for(itt=dp[p].S[j].begin();itt!=dp[p].S[j].end();++itt)
       ...
       dp[p].S[n-1].clear();
           for(itt=dp[!p].S[n-1].begin();itt!=dp[!p].S[n-1].end();++itt)
           {
               if(dp[!p].S[0].find(*itt)==dp[!p].S[0].end())
                dp[p].S[n-1].insert(*itt);
           }
           for(itt=dp[!p].S[0].begin();itt!=dp[!p].S[0].end();++itt)
           {
               if(dp[!p].S[n-1].find(*itt)==dp[!p].S[n-1].end())
                dp[p].S[n-1].insert(*itt);
           }
       pf("    ");
       for(itt=dp[p].S[n-1].begin();itt!=dp[p].S[n-1].end();++itt)
