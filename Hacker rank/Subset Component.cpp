...
int rec(int idx,ULL graph,int component)
{
    if(idx>=n)
    {
       return 64-popcount(graph)+component;
    }
    int sm=0;
    sm+=rec(idx+1,graph,component);
    if(ar[idx]==0LLU||(graph&ar[idx])!=0LLU)
    {
        sm+=rec(idx+1,graph|ar[idx],component);
    }
    else
        sm+=rec(idx+1,graph|ar[idx],component+1);
    return sm;
}
int main()
{
