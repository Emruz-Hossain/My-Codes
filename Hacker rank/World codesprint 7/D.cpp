...
ll ar[1000006],freq[1000006];
int main()
{
    //    freopen("output.txt","w",stdout);
    //    freopen("xinput.txt","r",stdin);
   //ios_base::sync_with_stdio(false);
   ll a,b,c,d,h,m,n,p,x,y,i,j,k,l,q,r,t,cnt,sm,tmp,increase,reduce,more;
   sf("%lld",&n);
   for(i=1;i<=n;i++)
    sf("%lld",&ar[i]);
    mem(freq,0);
   freq[1]=freq[n]=BigMod(2LL,n,mod);
      freq[1]--;
   if(n!=1)
    freq[n]--;
   cnt=0;
   ...
   while(i<=j)
   {
       increase=BigMod(2LL,n-2-cnt,mod);
       reduce=BigMod(2LL,cnt,mod);
       cnt++;
       more=((increase-reduce)%mod+mod)%mod;
       freq[i]=freq[j]=(freq[i-1]+more)%mod;
       i++;
       j--;
   }
   sm=0;
