...
int main()
{
   ...
   cnt=0;
   sf("%lld",&n);
   divisors_of[1].push_back(1);
   for(i=2;i<=n;i++)
   {
       divisors_of[i].push_back(1);
       divisors_of[i].push_back(i);

   }
   for(i=2;i<=n;i++)
   {
       for(j=2;i*j<n;j++)
            divisors_of[i*j].push_back(i);
   }
   ll ax,bx;
   for(ax=1;ax<=n;ax++)
   {
       bx=n-ax;
       for(i=0;i<divisors_of[ax].size();i++)
       {
           for(j=0;j<divisors_of[bx].size();j++)
           {
               if(divisors_of[ax][i]<divisors_of[bx][j])
                 S[divisors_of[ax][i]].push_back(divisors_of[bx][j]);
           }
       }
