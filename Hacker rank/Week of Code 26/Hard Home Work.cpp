...
int R[]={1,-1,0,0,1,-1,-1,1};  //move in 8 direction
...
double dp[3000005],S[3000005],C[3000005];
int main()
{
   ...
   for(i=1;i<=n;i++)
    ...
    double ans2=0.0;
   for(i=1;i<min(n-1,500);i++)
   {
       for(j=1;i+j<n&&j<=500;j++)
       {
           k=n-i-j;
           if(k>0)
           ans2=max(ans2,dp[i]+dp[j]+dp[k]);
       }
   }
   //pf("%0.9lf\n",ans2);
   S[2]=sin(1.0);
   ...
   for(i=4;i<=n;i++)
   {
       S[i]=sin((double)i/2.0);
       C[i]=max(C[i-2],cos((double)(i-2)/2.0));
   }
   double ans=0.0;
   for(i=2;i<n;i++)
   {
       ans=max(ans,2.0*S[i]*C[i]+dp[n-i]);
   }
   pf("%.9lf\n",ans);
    return 0;
}
