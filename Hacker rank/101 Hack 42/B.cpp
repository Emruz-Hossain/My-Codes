...
bool Line(points a, points b, points c)
{
    if((a.x-c.x)*(b.y-a.y)==(a.x-b.x)*(c.y-a.y))
        return true;
    else
        return false;
}
int main()
{
   ...
   while(t--)
   {
       ...
       for(i=0;i<n;i++)
       {
           if(Line(A,B,ar[i])||Line(A,C,ar[i])||Line(B,D,ar[i])||Line(C,D,ar[i]))
            cnt++;
       }
       if(cnt==n)
        pf("YES\n");
