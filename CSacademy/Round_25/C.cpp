...
const int sz=1005;
int n,m,fc,fr,sc,sr,h,w;
int dp[sz][sz],dist[sz][sz],Row[sz][sz],Col[sz][sz],ar[sz][sz];
bool possible(int x,int y)
{
    if((dp[x+h][y+w]-(dp[x-1][y+w]+dp[x+h][y-1]-dp[x-1][y-1]))>0)
        return false;
    else
        return true;
}
void dfs(void)
{
    ...
    while(!Q.empty())
    {
        ...
        for(i=0;i<4;i++)
        {
            x=u.first+R[i];
            y=u.second+C[i];
            if(x>=1&&x+h<=n&&y>=1&&y+w<=m&&possible(x,y))
            {
                if(dist[x][y]>dist[u.first][u.second]+1)
                {
                  // pf("%d %d\n",x,y);
...
int main()
{
   ...
   for(i=1;i<=n;i++)
   {
       for(j=1;j<=m;j++)
       {
           dp[i][j]=dp[i-1][j-1]+Row[i][j]+Col[i][j]-ar[i][j];
           //pf("%d ",dp[i][j]);
       }
      // pf("\n");
   }
   sf("%d %d %d %d %d %d",&h,&w,&sr,&sc,&fr,&fc);
   h--;w--;
   dfs();
   if(dist[fr][fc]!=inf)
    pf("%d\n",dist[fr][fc]);
