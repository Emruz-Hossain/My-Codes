...
ll dp[31][31][901],n,m;
ll rec(ll row,ll col,ll remaining)
{
    if(remaining==0)
        return 1;
    if(row==0||col==0)
        return 0;
    ll &ret=dp[row][col][remaining];
    if(ret!=-1)
    return ret;
    ll sm=0;
    sm=rec(row-1,col-1,remaining-1)*col;
    sm+=rec(row-1,col,remaining);
    return ret=sm;
}
int main()
{
   ...
   ll a,b,c,d,h,p,x,y,i,j,k,l,q,r,t,cnt,sm,tmp;
   readl(t);
   for(x=1;x<=t;x++)
   {
       readl2(n,m);
