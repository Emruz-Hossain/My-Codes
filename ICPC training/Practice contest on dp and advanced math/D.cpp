...
int rec(int egg,int floor)
{
    ...
    for(i=1;i<=floor;i++)
    {
        const int if_break=rec(egg-1,i-1);
        const int if_not_break=rec(egg,floor-i);
        mx=1+(if_break>if_not_break?if_break:if_not_break);
        sm=(sm<mx?sm:mx);
    }
    return ret=sm;
}
int main()
{
   ...
   int a,b,c,d,h,m,n,p,x,y,i,j,k,l,q,r,t,cnt,sm,tmp,egg,floor;
   sf("%d",&t);
   for(i=1;i<=1000;i++)
    dp[1][i]=i;
   for(i=2;i<=50;i++)
