...
int helth[20],damage[20][20],dp[65537],n;
char ss[20][20];
int rec(int mask)
{
    int i,j;
    ...
    for(i=0;i<n;i++)
    {
        if(Check(mask,i))
        {
            sm=min(sm,rec(Reset(mask,i))+helth[n-i-1]);
            for(j=0;j<n;j++)
            {
                if(Check(mask,j)==0&&damage[n-j-1][n-i-1]>0)
                {
                    int p=helth[n-i-1]/damage[n-j-1][n-i-1];
                    if(p*damage[n-j-1][n-i-1]<helth[n-i-1])
                        p++;
                    sm=min(sm,rec(Reset(mask,i))+p);
                }
            }
        }
    }
