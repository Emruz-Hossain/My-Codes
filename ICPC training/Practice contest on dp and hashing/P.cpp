...
int dp[1<<16],ar[105][20],visit[1<<16],n,m;
int toggle(int mask,int idx)
{
    int i,p;
    for(i=0;i<n;i++)
    {
...
int rec(int mask)
{
    ...
    for(i=0;i<m;i++)
    {
        int tmp_mask=toggle(mask,i);
        sm=min(sm,rec(tmp_mask)+1);
    }
    return ret=sm;
}
int main()
{
   ...
   int a,b,c,d,h,p,x,y,i,j,k,l,q,r,t,cnt,sm,tmp,pwr;
   read(t);
   for(x=1;x<=t;x++)
   {
       read2(n,m);
