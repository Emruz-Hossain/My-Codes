...
int taken,root=0,last_root,k,last_bad;
char ss[15005],is_good[50];
int main()
{
    //    freopen("output.txt","w",stdout);
    //    freopen("xinput.txt","r",stdin);
   //ios_base::sync_with_stdio(false);
   int a,b,c,d,h,m,n,p,x,y,i,j,l,q,r,t,cnt,sm,tmp,root,rt,ch;
    sf("%s",&ss);
    sf("%s",&is_good);
    sf("%d",&k);
    l=strlen(ss);
    ...
    {    rt=0;
        ...
        {
            ch=ss[j]-'a';
            bad_character+=(1-(is_good[ch]-'0'));
            if(bad_character>k)
                break;
           if(trie[rt].ar[ch].endm!=1)
               sm++;
           trie[rt].ar[ch].endm=1;
           if(trie[rt].ar[ch].ch==-1)
           {
               rt=trie[rt].ar[ch].ch=root;
               root++;
           }
           else
           {
