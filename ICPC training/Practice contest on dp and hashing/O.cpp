...
ll dp[105][105],ar[105],visit[105][105];
ll sum(int beg,int endd)
{
    ll i;
    ll sm=0;
    for(i=beg;i<=endd;i++)
...
ll rec(ll beg,ll endd)
{
    ll i,sm,tmp1,tmp2;
    if(beg==endd)
        return ar[beg];
    if(visit[beg][endd]==1LL)
        return dp[beg][endd];
     sm=sum(beg,endd);
    tmp1=ar[beg];
...
int main()
{
   ...
   ll a,b,c,h,m,n,x,y,i,j,k,l,q,r,t,tmp;
  scanf("%lld",&t);
  long long cnt,d,p;
   for(x=1;x<=t;x++)
   {
