...
int main()
{
   ...
   for(i=0;i<n;i++)
   {
       read2(ar[i].x1,ar[i].y1);
       read2(ar[i].x2,ar[i].y2);
       ar[i].m=(double)(ar[i].y1-ar[i].y2)/(double)(ar[i].x1-ar[i].x2);
   }
   cnt=0;
   for(i=0;i<n;i++)
   {
       for(j=i+1;j<n;j++)
       {
           if((ar[i].x1==ar[j].x1&&ar[i].y1==ar[j].y1)||(ar[i].x2==ar[j].x1&&ar[i].y2==ar[j].y1)||(ar[i].x2==ar[j].x2&&ar[i].y2==ar[j].y2))
           {
               if((fabs(ar[i].m*ar[j].m)+1)<eps)
                cnt++;
