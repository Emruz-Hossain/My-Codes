...
int isNotRoot[sz],root;
vector<int> dfs(int rt)
{
    int i;
    vector<int>leftHalf,rightHalf,Total;
    if(tree[rt].l!=-1)
    {
        leftHalf=dfs(tree[rt].l);
    }
    if(tree[rt].r!=-1)
    {
        rightHalf=dfs(tree[rt].r);
    }
    for(i=0;i<leftHalf.size();i++)
    {
        if(leftHalf[i]>=tree[rt].v)
            break;
        else
            Total.push_back(leftHalf[i]);
    }
    Total.push_back(tree[rt].v);
    for(i=0;i<rightHalf.size();i++)
    {
        if(rightHalf[i]<tree[rt].v)
            continue;
        else
            Total.push_back(rightHalf[i]);
    }
    for(i=0;i<leftHalf.size();i++)
        pf("%d ",leftHalf[i]);
    pf("\n");
