...
vector<int>Edge[55];
const int mxsz=1<<16;
int visited[20][mxsz],possible_by_one_assasian[mxsz],dp[mxsz];
void dfs(int idx,int mask)
{
    int i,u,tmp_mask;
    if(visited[idx][mask]==1)
        return;
    visited[idx][mask]=1;
    possible_by_one_assasian[mask]=1;
...
int rec(int mask)
{
    ...
    if(ret!=-1)
        return ret;
    int sm=100;
    for(i=mask;i>0;i=mask&(i-1))
    {
        if(possible_by_one_assasian[i])
        {
            sm=min(sm,1+rec(mask^i));
        }
    }
    return ret=sm;
}
int main()
{
   ...
   for(x=1;x<=t;x++)
   {
       ...
       mem(visited,0);
       mem(possible_by_one_assasian,0);
       for(i=0;i<n;i++)
       {
           dfs(i,1<<i);
       }
