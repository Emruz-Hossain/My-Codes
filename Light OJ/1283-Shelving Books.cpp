...
int rec(int idx,int left,int right)
{
    ...
    if(ret!=-1)
        return ret;
    int sm=0;
    if(ar[left]<=ar[idx]&&ar[idx]<=ar[right])
        sm=max(sm,rec(idx+1,idx,right)+1);
    if(ar[right]>=ar[idx]&&ar[idx]>=ar[left])
        sm=max(sm,rec(idx+1,left,idx)+1);
    sm=max(sm,rec(idx+1,left,right));
    return ret=sm;
}
int main()
{
