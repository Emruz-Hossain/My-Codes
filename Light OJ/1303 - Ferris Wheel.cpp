...
vector<int>Q;
int n,m;
int riding[25],ridden[25][25],total_ridden[25];
bool finished()
{
    for(int i=0;i<n;i++)
        if(total_ridden[i]<m)
...
int main()
{
   ...
   int a,b,c,d,h,p,x,y,i,j,k,l,q,r,t,cnt,sm,tmp,current_seat;
   read(t);
   for(x=1;x<=t;x++)
   {
       ...
       while(!finished())
       {
           p=riding[current_seat];
           if(p!=-1)
           {
               riding[current_seat]=-1;
               a=total_ridden[p];
               if(a<m)
                Q.push_back(p);
           }
          ...
          {
              Q.erase(Q.begin()+i);
              riding[current_seat]=p;
              ridden[p][current_seat]=1;
              total_ridden[p]++;
          }
          sm+=5;
          current_seat++;
