...
int main()
{
   ...
   ll sm;
   powr[0]=1;
   adjacent_bit[0]=adjacent_bit[1]=0;
   for(i=2;i<=32;i++)
   {
       adjacent_bit[i]=powr[i-2]+adjacent_bit[i-1]*2;
       powr[i-1]=powr[i-2]*2;
   }
   sf("%d",&t);
   for(x=1;x<=t;x++)
   {
       ...
       for(i=l-1;i>=0;i--)
       {
           if(v[i]==1)
           {
               taken=taken+powr[i];
               sm+=adjacent_bit[i];
               sm+=constant_adjacent_pair*(taken-last_taken);
               last_taken=taken;
               if(i<l-1&&v[i+1]==1)
               {
                     sm++;
                     constant_adjacent_pair++;
               }
