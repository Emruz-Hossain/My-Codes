...
PLL dp[2][30][30][30][3][7];
ll ideal[35],binary[35],start,endd,MaxOne,k,Num;
PLL rec(ll isLess,ll idx,ll ones,ll diff,ll mod3,ll mod7)
{
    int i,last;
    if(idx<0LL)
    {
        PLL ans;
        ans.fs=0;ans.sd=0;
        if(ones<=MaxOne&&diff<=k&&mod3==0LL&&mod7!=0LL)
        {
            ans.sd=1;
            //pf("%lld\n",diff);
        }
    ...
    PLL &ret=dp[isLess][idx][ones][diff][mod3][mod7];
    if(ret.sd!=-1)
        return ret;
    PLL sm;
    sm.fs=0;sm.sd=0;
    ...
    for(i=0;i<=last;i++)
    {
        PLL d=rec(isLess|(i<last),idx-1,ones+i,diff+!(i==ideal[idx]),(mod3+(i*(1LL<<idx)))%3,(mod7+(i*(1LL<<idx)))%7);
        sm.fs+=d.fs;
        sm.sd+=d.sd;
        sm.fs+=i*(1LL<<idx)*d.sd;
    }
    return ret=sm;
}
ll cal(ll n)
...
int main()
{
    ...
    ll a,b,c,d,h,m,n,p,x,y,i,j,l,q,r,t,cnt,sm,tmp;
    scanf("%lld",&t);
    for(x=1;x<=t;x++)
    {
        scanf("%lld %lld %lld %lld %lld",&start,&endd,&MaxOne,&Num,&k);
        MaxOne=min(MaxOne,30LL);
        k=min(k,30LL);
        for(i=0;i<30LL;i++)
        {
            ideal[i]=Check(Num,i);
        }
        printf("Case %lld: %lld\n",x,cal(endd)-cal(start-1));
    }
