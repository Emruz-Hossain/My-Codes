...
int dp[1<<17],n;
int SameLine(int i,int j,int k)
{
    int x1=ar[i].x,x2=ar[j].x,x3=ar[k].x,y1=ar[i].y,y2=ar[j].y,y3=ar[k].y;
    if((x1-x3)*(y2-y1)==(x1-x2)*(y3-y1))
        return 1;
    else
        return 0;
}
        ...
        if(Check(mask,i)==0)
        {
            ...
            for(j=i+1;j<n;j++)
            {
                if(Check(mask1,j)==0)
                {
                    ...
                    for(k=j+1;k<n;k++)
                    {
                        if(Check(mask2,k)==0&&SameLine(i,j,k))
                        {
                            mask2=Set(mask2,k);
                        }
                    }
                    sm=min(sm,1+rec(mask2));
                }
            }
            break;
        }
