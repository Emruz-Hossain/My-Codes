...
int tree[8*100001],visit[2*100002];
  int cnt;
...
void update(int rt,int beg,int endd,int l,int r,int p)
{
    if(beg>=l&&endd<=r)
    {
        tree[rt]=p;
    ...
    if(tree[rt]!=0)
       propagate(rt);
    int mid=(beg+endd)/2;
    update(2*rt,beg,mid,l,r,p);
    update(2*rt+1,mid+1,endd,l,r,p);
}
void query(int rt,int beg,int endd)
{
    if(tree[rt]!=0)
    {
        if(!visit[tree[rt]])
...
int main()
{
   ...
   int a,b,c,d,h,m,n,p,x,y,i,j,k,l,q,r,t,sm,tmp;
   readfast(t);
   for(x=1;x<=t;x++)
   {
       readfast(n);
