...
vector<st>Vocabolary;
vector<int>ar;
char ss[100005];
int rec(int idx,vector<int>rem)
{
    int cnt=0;
    int i,j,p=0;
    if(idx>=Vocabolary.size())
    {
        cnt=0;
        for(i='a';i<='z';i++)
           cnt+=rem[i];
       if(cnt==0)
        ...
        for(j='a';j<='z';j++)
        {
            if(temp[j]>=Vocabolary[idx].freq[j])
                temp[j]-=Vocabolary[idx].freq[j];
            else
            {
                flag=0;
                break;
...
int main()
{
   ...
   for(x=1;x<=t;x++)
   {
       ...
       {
           for(j='a';j<='z';j++)
            Vocabolary[i].freq[j]=0;
       }
       for(i=0;i<n;i++)
       {
           sf("%s",&ss);
           for(j=0;j<strlen(ss);j++)
            Vocabolary[i].freq[ss[j]]++;
       }
       vector<int>ans;
       for(i=0;i<m;i++)
       {
