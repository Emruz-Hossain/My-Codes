...
int main()
{
   ...
   for(i=0;i<m;i++)
    sf("%lld",&power_of_socket[i]);
   sort(temp.begin(),temp.endd());
   ...
   {
       p=bs(ar[i]);
       computer_with_power[u].push_back(i);
   }
   for(i=0;i<n;i++)
   {
       adapter_need[i]=lmax;
   ...
   for(i=0;i<m;i++)
   {
       p=power_of_socket[i];
       p=bs(p);
       if(computer_with_power[p].size()>0)
       {
           a=computer_with_power[p].back();
           computer_with_power[p].pop_back();
           adapter_of_socket[i]=0;
