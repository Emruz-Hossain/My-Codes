...
vector<int>tem,v;
int language[1000006],ns[1000006],audio[200005],subtitle[200005];
bool cmp(st a,st b)
{
    if(a.very==b.very)
        return a.al>b.al;
...
int main()
{
   ...
   {
       sf("%d",&language[i]);
       tem.push_back(language[i]);
   }
   sf("%d",&m);
   for(i=0;i<m;i++)
   {
       sf("%d",&audio[i]);
       tem.push_back(audio[i]);
   }
   for(i=0;i<m;i++)
   {
       sf("%d",&subtitle[i]);
       tem.push_back(subtitle[i]);
   }
   sort(tem.begin(),tem.end());
   l=tem.size();
   for(i=0;i<l;i++)
   {
       if(v.size()==0||tem[i]!=v.back())
        v.push_back(tem[i]);
   }
   mem(ns,0);
   for(i=0;i<n;i++)
