...
int rec(int start,int endd)
{
    ...
    if(start==endd)
        return 1;
    if(start+1==endd&&v[start]==v[endd])
        return 1;
   int &ret=dp[start][endd];
   if(ret!=-1)
    return ret;
   int mx=rec(start+1,endd)+1;
   if(v[start]==v[start+1])
    mx=min(mx,rec(start+2,endd)+1);
   for(i=start+2;i<=endd;i++)
   {
       if(v[i]==v[start])
        mx=min(mx,rec(start+1,i-1)+rec(i+1,endd));
   }
   return ret=mx;
}
int main()
