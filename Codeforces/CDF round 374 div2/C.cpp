...
int const mx=5005;
vector<ll>g[mx],rg[mx],cost[mx],rcost[mx],path;
struct node
{
    ll u,w;
...
st rec(ll idx,ll rem)
{
    ...
    {
        ll v=rg[idx][i];
        if(d[v]<=rem-rcost[idx][i])
        {
            q=rec(v,rem-rcost[idx][i]);
            if(q.v>p.v)
            {
                p.v=q.v;
                p.par=v;
...
int main()
{
   ...
   ll a,b,c,d,h,m,n,p,x,y,i,j,k,l,q,r,t,cnt,sm,tmp,u,v,w;
