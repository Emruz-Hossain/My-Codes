...
const int sz=105;
ll crush[sz],cycle_len_of[sz],visit[sz],n;
void find_cycle(ll cur,ll root,ll len)
{
    if(visit[cur]==1LL&&cur==root)
    {
        cycle_len_of[root]=len;
        return;
    }
    if(visit[cur]==1LL)
        return;
    visit[cur]=1;
    find_cycle(crush[cur],root,len+1);
}
...
int main()
{
   ...
   ll a,b,c,d,m,p,x,y,i,j,k,q,r,t,cnt,sm,tmp;
   sf("%lld",&n);
   for(i=1;i<=n;i++)
    sf("%lld",&crush[i]);
   mem(cycle_len_of,-1);
  ...
  ll ans=0;
   for(i=1;i<=n;i++)
   {
       if(cycle_len_of[i]==-1LL)
       {
           ans=-1;
           break;
       }
       if(cycle_len_of[i]%2==0LL)
        cycle_len_of[i]/=2;
   }
   if(ans!=-1LL)
   {
