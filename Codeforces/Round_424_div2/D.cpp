...
const int sz=100005;
ll loc,n,k,persion[sz],key[sz],visit[sz];
bool cmp1(ll a, ll b)
{
    return abs(loc - a) > abs(loc - b);
}
bool possible(ll mid)
{
    ...
    for (i = 0; i < k;i++)
    {
        ...
        for (j = 0; j < n; j++)
        {
            if(visit[j]==0&&abs(persion[j]-key[i])+abs(key[j]-loc)<=mid&&abs(persion[j]-key[i])+abs(key[j]-loc)>mx)
            {
                flag = 1;
                idx = j;
                mx = abs(persion[j] - key[i]) + abs(key[j] - loc);
            }
        }
        if(flag)
        {
...
int main()
{
   ...
   ll a,b,c,d,h,m,p,x,y,i,j,l,q,r,t,cnt,sm,tmp;
