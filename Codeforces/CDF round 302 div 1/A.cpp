...
double ART(P ae,P be){return crp(ae,be)/2.0;}
 ...
 int dp[2][505][505],bug[505],m,n,b,MOD;
...
int main()
{
   ...
   int a,c,d,h,p,x,y,i,j,k,l,q,r,t,cnt,sm,tmp;
   read2(n,m);
   read2(b,MOD);
   for(i=1;i<=n;i++)
    read(bug[i]);
   ...
   for(i=1;i<=n;i++)
   {
       ...
       for(j=1;j<=m;j++)
       {
           for(k=0;k<=b;k++)
           {
               if(k-bug[i]>=0&&dp[cur][j-1][k-bug[i]]>0)
                dp[cur][j][k]=(dp[cur][j][k]+dp[cur][j-1][k-bug[i]])%MOD;
           }
       }
       tmp=cur;
       cur=prev;
