...
int main()
{
   ...
   for(i=0;i<n;i++)
   {
       ar[i].fs=bs(ar[i].fs)+1;
       ar[i].sd=bs(ar[i].sd)+1;
   }
   mx=vv.size();
   sort(ar,ar+n,cmp1);
   mem(tree,0);
   for(i=0;i<n;i++)
   {
       ar[i].ans=query(ar[i].second)-query(ar[i].first-1);
      // pf("%d %d = %d\n",ar[i].first,ar[i].second,ar[i].ans);
       update(ar[i].first,1);
   }
   sort(ar,ar+n,cmp2);
   for(i=0;i<n;i++)
