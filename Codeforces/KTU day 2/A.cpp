...
double qube(double n)
{
    double l=0.0,h=n,mid;
    for(int i=0;i<400;i++)
    {
        mid=(l+h)/2;
        if(mid*mid*mid>n+eps)
            h=mid;
        else
            l=mid;
    }
...
int main()
{
  ...
  double a,b,c,d,h,m,n,p,x,y,j,k,l,q,r,t,cnt,sm,tmp,mid;
   sf("%lf %lf %lf",&a,&b,&p);
   l=0;h=9000000000000000000.0;
for(int i=0;i<500;i++)
   {
      mid=(l+h)/2.0;
       d=a*qube(mid)+b*sqrt(mid);
       if(d+eps>p)
       {
           h=mid;
       }
