...
ll ar[100005],sum[100005],query[100005];
int main()
{
   ...
   mem(br,0);
    ...
    for(i=n;i>0;i--)
    {
        ...
        d=query[i];
        if(br[d-1].have&&br[d+1].have)
        {
            br[d].have=1;
            p=br[d-1].left;
            q=br[d+1].right;
            b=br[p].right=br[d].right=q;
            a=br[q].left=br[d].left=p;
            Max_Heap.push(sum[b]-sum[a-1]);
        }
        else if(!br[d-1].have&&br[d+1].have)
        {
            br[d].have=1;
            p=br[d+1].right;
            b=br[d].right=p;
            a=br[p].left=br[d].left=d;
            Max_Heap.push(sum[b]-sum[a-1]);
        }
        else if(br[d-1].have&&!br[d+1].have)
        {
