...
void build(int rt,int beg,int endd)
{
    if(beg==endd)
    {
        tree[rt].lft=ss[beg-1];
        tree[rt].rht=ss[beg-1];
        tree[rt].op=0;
        return;
    }
    int mid=(beg+endd)/2;
    ...
    tree[rt].op=tree[2*rt].op+tree[2*rt+1].op;
    if(tree[2*rt].rht==tree[2*rt+1].lft&&tree[2*rt].rht=='.')
        tree[rt].op++;
    tree[rt].lft=tree[2*rt].lft;
    tree[rt].rht=tree[2*rt+1].rht;
}
void update(int rt,int beg,int endd,int pos,char val)
{
    if(beg==pos&&endd==pos)
    {
        tree[rt].lft=tree[rt].rht=val;
        tree[rt].op=0;
        return ;
    }
    int mid=(beg+endd)/2;
    if(pos<=mid)
        update(2*rt,beg,mid,pos,val);
    else
        update(2*rt+1,mid+1,endd,pos,val);
     tree[rt].op=tree[2*rt].op+tree[2*rt+1].op;
    if(tree[2*rt].rht==tree[2*rt+1].lft&&tree[2*rt].rht=='.')
        tree[rt].op++;
    tree[rt].lft=tree[2*rt].lft;
