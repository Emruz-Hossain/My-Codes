...
ll query(ll rt,ll beg,ll endd, ll l,ll r, ll idx)
{
    if(beg>r||endd<l)
        return 0LL;
    if(beg>=l&&endd<=r&&tree[rt]<ar[idx])
    {
        return (endd-beg)+1;
    }
    ll mid=(beg+endd)/2;
    return query(2*rt,beg,mid,l,r,idx)+query(2*rt+1,mid+1,endd,l,r,idx);
}
int main()
{
