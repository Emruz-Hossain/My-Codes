...
vector<int>child[100005],nodes_of[100005];
int level[100005],max_lev,subtree_size[100005];
double Expt[100005];
void rec(int node,int lev)
{
    level[node]=lev;
    for(int i=0;i<child[node].size();i++)
    {
        rec(child[node][i],lev+1);
    }
}
int subtree(int node)
{
    ...
    for(int i=0;i<child[node].size();i++)
    {
        subtree_size[node]+=subtree(child[node][i]);
    }
...
int main()
{
   ...
   for(i=1;i<=n;i++)
   {
       Expt[i]=0.5*(n-subtree(i)-level[i]+1)+level[i];
      // pf("%d\n",subtree(i));
   }
   for(i=1;i<=n;i++)
   {
