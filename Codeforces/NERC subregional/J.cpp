...
int main()
{
   ...
   for(i=0;i<n;i++)
   {
    sf("%d",&br[i]);
       ...
       {
           dp[j].bottle=inf;
           dp[j].volume_of_bottles=0;
       }
       dp[0].bottle=0;
       for(i=0;i<n;i++)
       {
           for(j=10000;j>=br[i];j--)
           {
               if(dp[j].bottle>dp[j-br[i]].bottle+1)
               {
                   dp[j].bottle=dp[j-br[i]].bottle+1;
                   dp[j].volume_of_bottles=dp[j-br[i]].volume_of_bottles+ar[i];
               }
               else if(dp[j].bottle==dp[j-br[i]].bottle+1&&dp[j].volume_of_bottles<dp[j-br[i]].volume_of_bottles+ar[i])
               {
                   dp[j].bottle=dp[j-br[i]].bottle+1;
                   dp[j].volume_of_bottles=dp[j-br[i]].volume_of_bottles+ar[i];
               }
       ...
       {
           if(dp[i].bottle==mx)
            t=max(t,dp[i].volume_of_bottles);
       }
       pf("%d %d\n",mx,sm-t);
