...
double ART(P ae,P be){return crp(ae,be)/2.0;}
 ...
 int ar[100005],dominate[100005];
 struct st
 {
     int id,occ;
 }tree[4*100005];
inline void update(int rt,int beg,int endd,int idx,int val)
{
    if(beg==endd&&beg==idx)
    {
        tree[rt].occ+=val;
    ...
    if(tree[2*rt].occ>=tree[2*rt+1].occ)
    {
        tree[rt].occ=tree[2*rt].occ;
        tree[rt].id=tree[2*rt].id;
    }
    else
    {
        tree[rt].occ=tree[2*rt+1].occ;
        tree[rt].id=tree[2*rt+1].id;
    }
}
int main()
{
