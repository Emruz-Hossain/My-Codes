...
set<PII> rec(int beg,int endd)
{
    ...
    if(beg==endd)
    {
        set<PII>tmp;
        for(i=1;i<=k;i++)
            tmp.insert(make_pair(i,i*ar[beg]));
        //pf("%d\n",tmp.size());
        return tmp;
    }
    int mid=(beg+endd)/2;
    set<PII> S1=rec(beg,mid);
    set<PII> S2=rec(mid+1,endd);
   // pf("%d %d\n",S1.size(),S2.size());
...
//                break;
//            pf("sdlkfj\n");
            tmp.insert(make_pair(p1.fs+p2.fs,p1.sd+p2.sd));
        }
    }
    return tmp;
}
int main()
{
   ...
   int a,b,c,d,h,m,p,x,y,i,j,l,q,r,t,cnt,sm,tmp;
   sf("%d %d",&n,&k);
   for(i=0;i<n;i++)
    sf("%d",&ar[i]);
   set<PII> S=rec(0,n-1);
