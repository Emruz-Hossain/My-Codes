...
int main()
{
  ...
  unsigned long long a,b,c,d,h,m,n,p,x,y,i,j,k,l,q,r,t,cnt,tmp;
  sf("%I64u",&n);
   for(i=0;i<n;i++)
   {
      sf("%I64u %I64u",&ar[i].x,&ar[i].y);
       if(ar[i].x>ar[i].y)
       {
           tmp=ar[i].y;
           ar[i].y=ar[i].x;
   ...
   unsigned long long width=0,length=0;
   for(i=0;i<n;i++)
   {
      unsigned long long mx=1000000000005;
       for(j=i;j<n;j++)
       {
           mx=min(mx,ar[j].y);
       }
