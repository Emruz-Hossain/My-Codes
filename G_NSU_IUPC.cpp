...
int update(int rt,int beg,int endd,int pos)
{
    if(beg==endd)
    {
        tree[rt]=0;
    ...
    int p;
    if(tree[2*rt]>=pos)
        p= update(2*rt,beg,mid,pos);
    else
        p= update(2*rt+1,mid+1,endd,pos-tree[2*rt]);
    tree[rt]=tree[2*rt]+tree[2*rt+1];
    return p;
}
...
int main()
{
   ...
   for(x=1;x<=t;x++)
   {
       sf("%d",&n);
       for(i=0;i<n;i++)
        sf("%d %d",&ar[i].h,&ar[i].before_me);
       sort(ar,ar+n,cmp);
       build(1,1,n);
       int valid=1;
       for(i=0;i<n;i++)
       {
           p=update(1,1,n,ar[i].before_me+1);
           pf("%d %d\n",ar[i].h,p);
           if(p>n)
            valid=0;
           ar[i].pos=p;
