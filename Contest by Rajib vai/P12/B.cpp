...
ll BIGMOD(ll base,ll power,ll M)
{
    ...
    if(power%2==0LL)
    {
        ans=BIGMOD(base,power/2,M);
        return ans=(ans*ans)%M;
    }
    else
    {
        ans=BIGMOD(base,power-1,M);
        return ans=(ans*base)%M;
    }
}
int main()
{
   ...
   while(t--)
   {
       ...
       for(r=1;r<k;r++)
       {
           ll wrong_way=BIGMOD(r,n,mod);
           for(i=1;i<r;i++)
           {
               wrong_way=(wrong_way-(ncr[r][i]*BIGMOD(i,n,mod))%mod+mod)%mod;
           }
           sm=(sm-(ncr[k][r]*wrong_way)%mod+mod)%mod;
       }
       pf("%lld\n",sm);
   }
