...
struct RevengeOfTheSith{
    vector<int>dist;
    int mxd,dp[55][55],N;
    int rec(int idx,int last_height,int rem)
    {
        int d,cost;
        if(idx>=N)
            return 0;
        int &ret=dp[idx][rem][last_height];
        if(ret!=-1)
            return ret;
        int sm=inf;
        if(last_height>=dist[idx])
            cost=0;
        else
        {
            d=last_height+mxd;
            ...
            {
                cost=((dist[idx]-last_height)-mxd)*((dist[idx]-last_height)-mxd);
            }
        }
        sm=min(sm,rec(idx+1,dist[idx],rem)+cost);
        if(rem>0&&idx<N-1)
        {
            d=last_height+mxd;
            sm=min(sm,rec(idx+1,d,rem-1));
        }
        return ret=sm;
