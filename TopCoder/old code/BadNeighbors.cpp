...
struct BadNeighbors{
    int ar[50],n,dp[50][50][3];
    int rec(int cur,int prev,int flag)
    {
        int p=0,q=0;
        if(cur>=n)
            return 0;
        ...
        p=rec(cur+2,cur,flag)+ar[cur];
        q=rec(cur+1,prev,flag);
        return dp[cur][prev][flag]=max(p,q);
    }
int maxDonations(vector <int> donations)
{
    ...
    for(i=0;i<donations.size();i++)
        ar[i]=donations[i];
    mem(dp,-1);
    return ret=max(rec(1,0,0),rec(2,0,1)+ar[0]);
}
