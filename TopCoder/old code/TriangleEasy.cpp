...
{
int ret=3,i,j,k,flag=0,flag2=0;
int connected[55][55];
mem(connected,0);

    for(i=0;i<x.size();i++)
    {
        connected[x[i]][y[i]]=connected[y[i]][x[i]]=1;
    }
    for(i=0;i<n;i++)
    {
        for(j=0;j<n;j++)
        {
            ...
            for(k=0;k<n;k++)
            {
                if(k==i||k==j)
                    continue;
                if(connected[i][j]==1&&connected[j][k]==1&&connected[k][i]==1)
                    return 0;
                if(connected[i][j]==1&&connected[j][k]==1)
                {
                    flag=1;
                }
                if(connected[i][j]==1||connected[j][k]==1||connected[k][i]==1)
