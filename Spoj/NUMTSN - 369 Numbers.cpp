...
int zero,v[100];
ll dp[2][51][18][18][18];
ll rec(int isLess,int idx,int three,int six,int nine)
{
    ...
    ll &ret=dp[isLess][idx][three][six][nine];
    if(isLess&&ret!=-1LL)
        return ret;
    int i,last;
    ll sm=0;
    last=(isLess==1)?9:v[idx];
        ...
        for(i=0;i<=last;i++)
        {
            sm=(sm+rec(isLess|(i<last),idx-1,three+(i==3),six+(i==6),nine+(i==9)))%mod;
        }
    if(isLess)
        ret=sm;
    return sm;
