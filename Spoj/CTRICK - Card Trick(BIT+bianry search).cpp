...
int main()
{
   ...
   int a,b,c,d,h,m,p,x,y,i,j,k,l,q,r,t,cnt,sm,tmp,need,pos,remaining,last_pos;
   read(t);
   while(t--)
   {
       read(n);
      ...
      // pf("%d\n",query(3));
       ...
       for(i=1;i<=n;i++)
       {
           int left=query(last_pos-1);
           int right=query(n)-query(last_pos);
           need=left+i;
