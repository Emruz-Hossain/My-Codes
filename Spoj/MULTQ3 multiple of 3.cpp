...
double ART(P ae,P be){return crp(ae,be)/2.0;}
 ...
 int get_element(int rt,int indx)
 {
     int n=tree[rt].prop;
     n=n%3;
     while(n--)
  ...
  int get_element2(int rt,int indx,int prop)
 {
     int n=tree[rt].prop+prop;
     n=n%3;
     while(n--)
     {
         indx--;
 ...
 void set_value(int rt)
 {
     int i;
     for(i=0;i<3;i++)
     {

         int p=get_element(2*rt,i);
         int q=get_element(2*rt+1,i);
         tree[rt].cnt[i]=p+q;
     }
     return;
