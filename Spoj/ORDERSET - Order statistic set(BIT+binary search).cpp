...
vector<int>v,vv;
int BIT[200005],mx,visit[200005];
int bs(int p)
{
    int l=0,h=vv.size()-1,mid=0;
    while(l<=h)
    {
        mid=(l+h)/2;
        if(vv[mid]==p)
...
int bs2(int p)
{
    int l=0,h=vv.size()-1,mid=0,ans=-1;
    while(l<=h)
    {
        mid=(l+h)/2;
        if(vv[mid]<=p)
...
int main()
{
   ...
   for(i=0;i<n;i++)
   {
       sf("%s %d",&ar[i].ss,&ar[i].val);
       if(ar[i].ss[0]=='I')
        v.push_back(ar[i].val);
   }
   sort(v.begin(),v.end());
   for(i=0;i<v.size();i++)
   {
