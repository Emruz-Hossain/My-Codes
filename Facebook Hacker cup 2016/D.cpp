...
vector<string>v;
string ss;
int prefix[302][302],dp[302][302][302],n;
int count_prefix(int a,int b)
{
    int cnt=0,l,i;
    l=min(v[a].size(),v[b].size());
    for(i=0;i<l;i++)
    {
        if(v[a][i]!=v[b][i])
            return cnt;
...
int rec(int prev,int cur,int rem)
{
    if(rem==0)
        return v[prev].size();
    if(cur>n)
        return inf;
    int &ret=dp[prev][cur][rem];
    if(ret!=-1)
        return ret;
    int sm=inf;
    int d=prefix[prev][cur];
    int lp=v[prev].size();
    int lc=v[cur].size();
    int op=(lp-d)+(lc-d);
    sm=min(sm,rec(cur,cur+1,rem-1)+op);
    sm=min(sm,rec(prev,cur+1,rem));
    return ret=sm;
}
int main()
{
