...
int main()
{
   ...
   while(sf("%s",&ss[n])==1)
   {
       if(strlen(ss[n])==1&&ss[n][0]=='#')
        break;
       for(i=0;i<strlen(ss[n]);i++)
       {
           if(mp[ss[n][i]]==0)
           {
             mp[ss[n][i]]=cnt;
   ...
   for(i=0;i<n;i++)
   {
       ...
       {
           int k=0;
           while(k<strlen(ss[i])&&k<strlen(ss[j])&&ss[i][k]==ss[j][k]) k++;
           if(k<min(strlen(ss[i]),strlen(ss[j])))
           {
               Edge[mp[ss[i][k]]].push_back(mp[ss[j][k]]);
               indegree[mp[ss[j][k]]]++;
           }
